  ...
  const filteredSales = useMemo(() => {
    let result = sales.filter((sale) => {
      ...
      const matchesSearch =
        !lowerSearch ||
        normalizeSearchText(
          property?.address || sale.externalPropertyAddress,
        ).includes(lowerSearch) ||
        normalizeSearchText(client?.name || sale.comprador).includes(
          lowerSearch,
        ) ||
        normalizeSearchText(seller?.name || sale.vendedor).includes(
          lowerSearch,
        ) ||
      ...
      const matchesStatus =
        filterStatus === "all" || sale.estado === filterStatus;
      const matchesPropertyType =
        filterPropertyType === "all" ||
        (filterPropertyType === "vinculada"
          ? !!sale.propiedadId
          : !sale.propiedadId);
      const matchesBuyer =
        !filterBuyer || sale.clientCompradorId === filterBuyer;
      const matchesSeller =
        !filterSeller || sale.propietarioId === filterSeller;
      const matchesAgent =
        !filterAgent ||
        normalizeSearchText(sale.inmoAgente || "").includes(
          normalizeSearchText(filterAgent),
        );
      return (
        matchesSearch &&
        matchesStatus &&
    ...
    result.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "precio":
          compare =
            (a.valorCierre || a.precioAcordado || a.precioPublicado) -
            (b.valorCierre || b.precioAcordado || b.precioPublicado);
          break;
        default:
          compare = 0;
      }
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Gestión completa de reservas y operaciones de venta.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        />
        <StatCard
          label="Com. brutas cobradas (USD)"
          value={formatCurrency(stats.grossCommissionsUsd, "USD")}
          icon={DollarSign}
          color="green"
        />
      </div>
                ...
                <div className="space-y-3">
                  {stageSales.map((sale) => {
                    const property = properties.find(
                      (p) => p.id === sale.propiedadId,
                    );
                    const client = clients.find(
                      (c) => c.id === sale.clientCompradorId,
                    );
                    return (
                      <div key={sale.id}>
                        <Card className="p-4 cursor-pointer hover:border-blue-300 transition-all active:scale-[0.98] group relative">
                          <div className="flex items-center justify-between mb-3">
                            <Badge
                              size="xs"
                              variant={getStatusVariant(sale.estado) as any}
                            >
                              {sale.estado}
                            </Badge>
                            <div className="flex items-center gap-2">
                  ...
                  {stageSales.length === 0 && (
                    <div className="h-24 border-2 border-dashed border-slate-100 dark:border-slate-800 rounded-xl flex items-center justify-center bg-slate-50 dark:bg-slate-800/50">
                      <p className="text-[10px] font-black text-slate-300 dark:text-slate-600 uppercase italic tracking-widest">
                        Sin operaciones
                      </p>
                    </div>
                  )}
                ...
                {filteredSales.map((sale) => {
                  ...
                  return (
                    <tr
                      ...
                      id={`sale-row-${sale.id}`}
                      className={cn(
                        "hover:bg-blue-500/5 dark:hover:bg-blue-500/5 transition-colors group cursor-pointer",
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1 mt-0.5">
                          {client?.name || sale.comprador || "Sin comprador"} •{" "}
                          {property?.title ||
                            sale.externalPropertyCode ||
                            sale.propiedadId ||
                            "Sin vincular"}
                        </p>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex flex-col text-xs">
                          <span className="font-bold text-slate-700 dark:text-slate-300">
                            Pub:{" "}
                            {formatCurrency(sale.precioPublicado, sale.moneda)}
                          </span>
                          {sale.valorOfertado !== undefined && (
                            <span className="text-blue-600 font-medium">
                              Ofertado:{" "}
                              {typeof sale.valorOfertado === "number"
                                ? formatCurrency(
                                    sale.valorOfertado,
                                    sale.moneda,
                                  )
                          ...
                          {sale.valorCierre !== undefined && (
                            <span className="text-green-600 font-medium">
                              Cierre:{" "}
                              {formatCurrency(sale.valorCierre, sale.moneda)}
                            </span>
                          )}
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        ...
                        {sale.grossCommissionUsd !== undefined && (
                      ...
                      <td className="px-4 py-3">
                        <Badge variant={getStatusVariant(sale.estado) as any}>
                          {sale.estado}
                        </Badge>
                        {sale.isCollected && (
...
function SaleOperationMenu({
  ...
  onLog: (log: Omit<ActivityLog, "id" | "createdAt">) => void;
  ...
  const changeStatus = (status: SaleStatus) => {
    ...
    onLog({
      ...
      title: `Operación ${sale.nombre || sale.id} marcada como ${status}`,
      entityId: sale.id,
    });
  ...
  const toggleCollected = () => {
    ...
    onLog({
      type: "sale",
      action: "updated",
      title: `Operación ${sale.nombre || sale.id} ${next ? "marcada como cobrada" : "desmarcada como cobrada"}`,
      entityId: sale.id,
    });
    showToast(
      next
        ? "Operación marcada como cobrada"
        : "Operación desmarcada como cobrada",
      "success",
    );
  };
...
function SaleFormModal({
  ...
  const hasManualProperty = !!(
    sale?.externalPropertyAddress ||
    sale?.externalPropertyLink ||
    sale?.externalPropertyCode
  );
  const [propertyMode, setPropertyMode] = useState<"existing" | "manual">(
    sale
      ? sale.propiedadId
        ? "existing"
        : hasManualProperty
  ...
  const handleCreateBuyer = () => {
    if (!newBuyerName.trim()) {
      showToast("El nombre del comprador es obligatorio", "error");
      return;
    }
    const newClient = {
      id: generateId("c"),
  ...
  const handleCreateSeller = () => {
    if (!newSellerName.trim()) {
      showToast("El nombre del vendedor es obligatorio", "error");
      return;
    }
    const newClient = {
      id: generateId("c"),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasComprador = !!(
      formData.clientCompradorId ||
      (formData.compradorManual && formData.compradorManual.trim())
    );
    ...
    if (propertyMode === "existing" && !formData.propiedadId)
      newErrors.propiedad = true;
    if (
      propertyMode === "manual" &&
      !(
        formData.externalPropertyAddress ||
        formData.externalPropertyLink ||
        formData.externalPropertyCode
      )
    )
      newErrors.propiedadManual = true;
    if (!formData.precioPublicado && formData.precioPublicado !== 0)
  ...
  const inputCls = (field: string) =>
    cn(
      "w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800/50 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500/20 transition-all",
      errors[field]
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-4xl relative z-10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom duration-300">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 shrink-0">
          <h2 className="font-black text-xl text-slate-900 dark:text-slate-100">
            {sale ? "Editar" : "Nueva"} Operación
          </h2>
          <button
            onClick={onClose}
        ...
        >
          <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <button
                      type="button"
                      onClick={() => setShowNewBuyerForm((v) => !v)}
                      className="w-5 h-5 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 flex items-center justify-center font-bold text-xs transition-all"
                  ...
                  <SearchableSelect
                    ...
                    onChange={(val) => {
                      setFormData({
                        ...formData,
                        clientCompradorId: val,
                        compradorManual: "",
                      });
                      setErrors((prev) => ({ ...prev, comprador: false }));
                    }}
                    options={clients.map((c) => ({
                      value: c.id,
                      label: c.name,
                  ...
                  {showNewBuyerForm && (
                    <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-xl space-y-2">
                      <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">
                        Nuevo Comprador rápido
                      </p>
                      <input
                        className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-blue-200 rounded-lg text-sm outline-none"
                        placeholder="Nombre *"
                        value={newBuyerName}
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 py-1.5 bg-blue-600 text-white text-xs font-bold rounded-lg hover:bg-blue-700"
                        >
                          Crear y Seleccionar
                        </button>
                        <button
                          ...
                          className="px-3 py-1.5 text-slate-500 dark:text-slate-400 text-xs font-bold rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/50"
                        >
                          Cancelar
                        </button>
                      </div>
                ...
                <div>
                  <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">
                    O ingresar nombre manual
                  </p>
                  <input
                    className={cn(inputCls("comprador"), "mb-2")}
                    placeholder="Nombre del comprador (texto libre)"
                    value={formData.compradorManual || ""}
                    onChange={(e) => {
                      setFormData({
                        ...formData,
                        compradorManual: e.target.value,
                        clientCompradorId: e.target.value
                          ? ""
                          : formData.clientCompradorId,
                      });
                      setErrors((prev) => ({ ...prev, comprador: false }));
                  ...
                  <input
                    ...
                    value={formData.compradorInmobiliaria || ""}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        compradorInmobiliaria: e.target.value,
                      })
                    }
                  />
                </div>
            ...
            <div>
              <label
                className={cn(
                  ...
                  errors.propiedad || errors.propiedadManual
                    ? "text-red-500"
                    : "text-slate-400 dark:text-slate-500",
                )}
              >
              ...
              <div className="flex gap-2 mb-3">
                <button
                  ...
                  onClick={() => {
                    ...
                    setErrors((prev) => ({ ...prev, propiedadManual: false }));
                  }}
                  className={cn(
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setErrors((prev) => ({ ...prev, propiedad: false }));
                  }}
                  className={cn(
                    "px-3 py-1.5 rounded-full text-xs font-bold border transition-all",
                    propertyMode === "manual"
              ...
              {propertyMode === "existing" ? (
                <div>
                  <SearchableSelect
                    ...
                    onChange={(val) => {
                      setFormData({ ...formData, propiedadId: val });
                      setErrors((prev) => ({ ...prev, propiedad: false }));
                    }}
                    options={properties.map((p) => ({
                      ...
                      subtitle:
                        ...
                        (p.propertyCode || p.code
                          ? ` · Cód: ${p.propertyCode || p.code}`
                          : ""),
                    }))}
                  ...
                  {selectedProperty && (
                    <div
                      className={cn(
                        "mt-2 p-3 rounded-xl border text-xs space-y-1",
                        errors.propiedad
                  ...
                  {errors.propiedad && (
                    <p className="text-xs text-red-500 font-medium mt-1">
                      Seleccione una propiedad existente
                    </p>
                  )}
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      className={inputCls("propiedadManual")}
                      value={formData.externalPropertyAddress || ""}
                      onChange={(e) => {
                        setFormData({
                          ...formData,
                          externalPropertyAddress: e.target.value,
                        });
                        setErrors((prev) => ({
                          ...prev,
                          propiedadManual: false,
                        }));
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      className={inputCls("")}
                      value={formData.externalPropertyLink || ""}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          externalPropertyLink: e.target.value,
                        })
                      }
                    />
                  </div>
                  <div>
                    ...
                    <input
                      className={inputCls("")}
                      value={formData.externalPropertyCode || ""}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          externalPropertyCode: e.target.value,
                        })
                      }
                    />
                  </div>
                  {errors.propiedadManual && (
                    <p className="text-xs text-red-500 font-medium md:col-span-3">
                      Completa al menos un dato de la propiedad manual
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowNewSellerForm((v) => !v)}
                    className="w-5 h-5 rounded-full bg-blue-100 text-blue-600 hover:bg-blue-200 flex items-center justify-center font-bold text-xs transition-all"
                  ...
                  <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-xl space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 text-slate-500 dark:text-slate-400 text-xs font-bold rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"
                      >
                        Cancelar
                      </button>
                    </div>
                ...
                <div className="mt-2">
                  ...
                  <input
                    className={inputCls("")}
                    placeholder="Nombre vendedor (texto libre)"
                    value={formData.vendedorManual || ""}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        vendedorManual: e.target.value,
                      })
                    }
                  />
                </div>
              ...
              <div>
                ...
                <SearchableSelect
                  ...
                  onChange={(val) =>
                    setFormData({ ...formData, vendedorId: val || undefined })
                  }
                  options={clients.map((c) => ({
                    value: c.id,
                    label: c.name,
                ...
                <div className="mt-2">
                  ...
                  <input
                    className={inputCls("")}
                    placeholder="Nombre de la inmobiliaria"
                    value={formData.vendedorInmobiliaria || ""}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        vendedorInmobiliaria: e.target.value,
                      })
                    }
                  />
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  className={inputCls("")}
                  value={formData.inmoAgente || ""}
                  onChange={(e) =>
                    setFormData({ ...formData, inmoAgente: e.target.value })
                  }
                />
              </div>
            </div>
              ...
              {[
                ...
                <div key={field.key}>
                  ...
                  <input
                    ...
                    value={
                      ((formData as Record<string, unknown>)[field.key] as
                        | string
                        | number) ?? ""
                    }
                    onChange={(e) => {
                      ...
                      setFormData({
                        ...formData,
                        [field.key]: val ? Number(val) : undefined,
                      });
                      if (field.required)
                        setErrors((prev) => ({ ...prev, [field.key]: false }));
                    }}
                  />
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">
                  % Bruto sobre Valor Cierre
                </label>
                <input
                  type="number"
                  className={inputCls("")}
                  value={formData.porcentajeBruto ?? ""}
                  onChange={(e) =>
                    handlePorcentajeBrutoChange(Number(e.target.value) || 0)
                  }
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  className={inputCls("")}
                  value={formData.grossCommissionUsd ?? ""}
                  onChange={(e) =>
                    handleGrossCommissionUsdChange(Number(e.target.value) || 0)
                  }
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="number"
                  className={inputCls("")}
                  value={formData.porcentajeReferido ?? ""}
                  onChange={(e) =>
                    handlePorcentajeReferidoChange(Number(e.target.value) || 0)
                  }
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  className={inputCls("")}
                  value={formData.montoReferido ?? ""}
                  onChange={(e) =>
                    handleMontoReferidoChange(Number(e.target.value) || 0)
                  }
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleGastosOficinaChange(Number(e.target.value) || 0)
                  }
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, fechaReserva: e.target.value })
                  }
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, fechaEscritura: e.target.value })
                  }
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, escribania: e.target.value })
                  }
                />
              </div>
              <div>
          ...
          <div className="mt-10 flex gap-3 sticky bottom-0 bg-white dark:bg-slate-900 pt-4 border-t border-slate-100 dark:border-slate-800">
            ...
            <Button
              type="submit"
              variant="primary"
              className="flex-1 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 border-none text-white shadow-lg shadow-blue-500/25"
            >
              {sale ? "Guardar Cambios" : "Registrar Operación"}
            </Button>
          </div>
        </form>
      </div>
