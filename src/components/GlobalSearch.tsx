...
export default function GlobalSearch() {
  ...
  const { clients, properties, tasks, sales, referredColleagues, buyers } = useAppContext();
  ...
  const results: SearchResultItem[] = useMemo(() => {
    return searchAll({ clients, properties, tasks, sales, referredColleagues, buyers }, query);
  ...
  const grouped = useMemo(() => {
    ...
    const order: SearchResultItem['type'][] = ['cliente', 'propiedad', 'venta', 'tarea', 'colega'];
    order.forEach(type => {
  ...
  return (
    <div className="relative max-w-sm w-full">
      <div className="flex items-center text-sm text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700">
        ...
        />
        {query && (
          <button onClick={handleClear} className="p-0.5 hover:bg-slate-200 dark:bg-slate-700 rounded flex-shrink-0">
          ...
          >
            {grouped.length === 0 ? (
              <div className="p-6 text-center">
                <p className="text-sm text-slate-400 dark:text-slate-500 font-medium">No se encontraron resultados</p>
              </div>
            ) : (
