...
export default function Properties() {
  ...
  const [captureStep, setCaptureStep] = useState<
    "idle" | "analyzing" | "error" | "manual" | "preview"
  >("idle");
  ...
  const filteredProps = properties
    .filter((p) => {
      ...
      const matchesOperation =
        !filterOperation ||
        p.operation === filterOperation ||
        (filterOperation === "ambas" && p.operation === "ambas") ||
        (filterOperation === "venta" && p.operation === "ambas") ||
        (filterOperation === "alquiler" && p.operation === "ambas");
  ...
  if (selectedProp) {
    ...
    const interestedClients = clients
      .filter(
        (c) =>
          (c.interestZone &&
            selectedProp.zone
              .toLowerCase()
              .includes(c.interestZone.toLowerCase())) ||
          (c.budget > 0 &&
            Math.abs(c.budget - selectedProp.price) / selectedProp.price < 0.2),
      )
      .slice(0, 5);
            ...
            <div className="relative group rounded-2xl overflow-hidden shadow-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                <Badge
                  ...
                  size="md"
                >
                  {getPropertyStatusLabel(selectedProp.status)}
                </Badge>
                <Badge variant="blue" size="md">
                  {selectedProp.operation}
                </Badge>
              </div>
            </div>
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="flex items-center text-slate-500 dark:text-slate-400 mt-2 font-medium">
                    ...
                    />
                    {selectedProp.address}, {selectedProp.zone},{" "}
                    {selectedProp.city}
                  </p>
                  {selectedProp.ownerId && (
                    <p className="text-sm text-blue-700 font-medium mt-1">
                      Dueño:{" "}
                      {clients.find((c) => c.id === selectedProp.ownerId)
                        ?.name || "ID: " + selectedProp.ownerId}
                    </p>
                  )}
                </div>
              ...
              <div className="mt-8">
                <EntityNotesPanel
                  ...
                  onDeleteNote={(noteId) => {
                    updateProperty({
                      ...selectedProp,
                      historyNotes: (selectedProp.historyNotes || []).filter(
                        (n) => n.id !== noteId,
                      ),
                    });
                  }}
          ...
          <div className="space-y-6">
            <Card title="Acciones de Propiedad">
              <div className="space-y-3">
                ...
                >
                  <MessageCircle size={18} className="mr-2" /> Enviar Ficha por
                  WhatsApp
                </Button>
                <Button
                  variant="primary"
                  className="w-full"
                  onClick={() =>
                    navigate("/agenda", { state: { prefillPropertyId: id } })
                  }
                >
                ...
                <Button
                  ...
                  className="w-full"
                  onClick={() =>
                    navigate("/tareas", { state: { prefillPropertyId: id } })
                  }
                >
                  <PlusCircle size={18} className="mr-2 text-blue-600" /> Crear
                  Tarea
                </Button>
                <Button
                  variant="ghost"
                  className="w-full border border-slate-100 dark:border-slate-800"
                  onClick={() => handleOpenForm(selectedProp)}
                >
                ...
                <div className="grid grid-cols-1 gap-3 pt-2">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      selectedProp.externalLink &&
                      window.open(selectedProp.externalLink, "_blank")
                    }
                  >
              ...
              <div className="space-y-3">
                {events
                  ...
                  .map((event) => (
                    <div
                      key={event.id}
                      className="flex items-start gap-3 p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                ...
                {events.filter((e) => e.propertyId === id).length === 0 && (
                  <p className="text-xs text-center text-slate-400 dark:text-slate-500 py-2 italic">
                    Sin eventos.
                  </p>
                )}
              </div>
                ...
                {interestedClients.map((client) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-bold text-slate-900 dark:text-slate-100 truncate group-hover:text-blue-600 transition-colors">
                        {client.name}
                      </p>
            ...
            <Button
              variant="outline"
              className="w-full"
              onClick={() => openRelations("property", selectedProp.id)}
            >
              <Link2 size={16} className="mr-2" /> Ver vínculos (Vista 360°)
            </Button>
            <RelationsPanel
              groups={getPropertyRelations(selectedProp.id, {
                clients,
                properties,
                sales,
                tasks,
              ...
              <div className="p-6 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-2">
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    if (!selectedProp) {
                      showToast(
                        "No se encontró la propiedad de referencia",
                        "error",
                      );
                      return;
                    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      const label = prompt(
                        "Nombre del nuevo tipo de propiedad:",
                      );
                      ...
                      const id = label
                        ...
                        .replace(/\s+/g, "_");
                      if (
                        customOptions.propertyTypes.some((t) => t.id === id)
                      ) {
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const id = label
                        ...
                        .replace(/\s+/g, "_");
                      if (
                        customOptions.propertyOperations.some(
                          (o) => o.id === id,
                        )
                      ) {
                        showToast("Ya existe esa opción", "warning");
                      ...
                      updateCustomOptions({
                        ...customOptions,
                        propertyOperations: [
                          ...customOptions.propertyOperations,
                          { id, label: label.trim() },
                        ],
                      });
                    }}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={() => {
                      const label = prompt(
                        "Nombre del nuevo estado de propiedad:",
                      );
                      if (!label?.trim()) return;
                      const id = label
                        .trim()
                        ...
                        .replace(/\s+/g, "_");
                      if (
                        customOptions.propertyStatuses.some((s) => s.id === id)
                      ) {
                        showToast("Ya existe esa opción", "warning");
                        return;
                      }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, propertyLink: e.target.value })
                  }
                />
              </div>
              <div>
                  ...
                  <div className="mt-3 p-3 border border-blue-100 rounded-xl bg-blue-50/50 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={handleCreateOwnerFromForm}
                      >
                        Agregar dueño
                      </button>
                      <button
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Cartera de propiedades de referencia y captación.
          </p>
        </div>
        <div className="flex gap-2">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 p-6">
            {filteredProps.map((prop) => {
              ...
              const imgSrc =
                prop.imageUrl || (prop.images && prop.images[0]) || "";
              return (
                <div
                  key={prop.id}
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setOpenMenuPropertyId(
                            openMenuPropertyId === prop.id ? null : prop.id,
                          );
                        }}
                      >
                        <MoreVertical
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 via-black/30 to-transparent">
                      <p className="text-white font-black text-lg drop-shadow-sm">
                  ...
                  <div className="p-4">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-slate-100 text-sm truncate group-hover:text-blue-600 transition-colors">
                      {prop.title}
                    </h3>
                    ...
                    {(prop.propertyCode || prop.code) && (
                      <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 font-medium">
                        Ref: {prop.propertyCode || prop.code}
                      </p>
                    )}
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1 font-medium">
                      Dueño:{" "}
                      {clients.find((c) => c.id === prop.ownerId)?.name ||
                        "Sin dueño asignado"}
                    </p>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-2xl relative z-10 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-700 font-medium">
                Extracción automática desde portales inmobiliarios. El sistema
                detecta precio, zona, ambientes y fotos directamente del link.
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">
                ...
                <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-wider font-bold">
                  Compatible con Zonaprop, Argenprop y Mercado Libre
                </p>
              </div>
              {captureStep === "idle" && (
                <Button
                  ...
                  onClick={async () => {
                    if (!captureUrl || !captureUrl.startsWith("http")) {
                      showToast(
                        "Ingresa una URL válida que comience con http",
                        "error",
                      );
                      return;
                    }
                    ...
                    steps.forEach((_, i) => {
                      setTimeout(
                        () => setCaptureProgressStep(i + 1),
                        (i + 1) * 500,
                      );
                    });
                    try {
                      const res = await fetch(
                        `/api/scrape?url=${encodeURIComponent(captureUrl)}`,
                      );
                      const data = await res.json();
                      if (!res.ok || !data.success) {
                        setCaptureErrorMsg(
                          data.error ||
                            `Error ${res.status}: El portal bloqueó la conexión.`,
                        );
                      ...
                      setCaptureErrorMsg(
                        err?.message ||
                          "Error de conexión con el servidor. El portal puede estar bloqueando solicitudes automáticas.",
                      );
                      setCaptureStep("error");
                      setCaptureProgressStep(0);
                    }
                ...
                <div className="p-6 bg-red-50 dark:bg-red-900/10 rounded-xl border border-red-200 dark:border-red-800/30 space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-sm font-bold text-red-800 dark:text-red-300">
                        El portal bloqueó la conexión automática
                      </h4>
                      <p className="text-xs text-red-600 dark:text-red-400 mt-1 leading-relaxed">
                        {captureErrorMsg ||
                          "No se pudo acceder al link automáticamente. Algunos portales (como Zonaprop) protegen sus publicaciones contra bots."}
                      </p>
                    </div>
                  </div>
                  <div className="bg-white dark:bg-slate-800/50 rounded-lg p-3 border border-red-100 dark:border-slate-700 space-y-2">
                    ...
                    <ul className="text-xs text-slate-500 dark:text-slate-400 space-y-1 list-disc list-inside">
                      <li>
                        Verificá que el link sea una publicación activa y
                        pública.
                      </li>
                      <li>Probá de nuevo en unos segundos.</li>
                      <li>
                        Usá el <strong>Modo Manual</strong> como fallback
                        infalible.
                      </li>
                    </ul>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="p-3 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-xl">
                    <p className="text-xs text-amber-800 dark:text-amber-300 font-medium leading-relaxed">
                      <strong>Modo de Captación Manual.</strong> Abrí el link en
                      tu navegador, seleccioná todo (
                      <kbd className="px-1 py-0.5 bg-white dark:bg-slate-800 rounded border border-amber-200 dark:border-amber-700 font-mono text-[10px]">
                        Ctrl+A
                      </kbd>{" "}
                      +{" "}
                      <kbd className="px-1 py-0.5 bg-white dark:bg-slate-800 rounded border border-amber-200 dark:border-amber-700 font-mono text-[10px]">
                      ...
                      en Zonaprop) y pegalo acá abajo. El sistema extraerá los
                      datos directamente del contenido.
                    </p>
                  </div>
                  <textarea
                    rows={8}
                  ...
                  <Button
                    ...
                    onClick={() => {
                      try {
                        ...
                        showToast(
                          "No se pudieron extraer datos del texto pegado. Verificá que sea el contenido completo de la publicación.",
                          "error",
                        );
                      }
                    }}
                ...
                <div className="space-y-4 border border-slate-200 dark:border-slate-700 rounded-xl p-4 bg-slate-50 dark:bg-slate-800/50">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-bold"
                        value={capturePreview.title || ""}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div>
                      ...
                      <select
                        ...
                        value={capturePreview.type || "departamento"}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                            type: e.target.value,
                    ...
                    <div>
                      ...
                      <select
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                        value={capturePreview.operation || "venta"}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div>
                      ...
                      <select
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                        value={capturePreview.currency || "USD"}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                        value={capturePreview.bedrooms || 1}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                        value={capturePreview.bathrooms || 1}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                        value={capturePreview.surface || 0}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div>
                      ...
                      <input
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                        value={capturePreview.externalLink || ""}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div>
                      ...
                      <input
                        className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm"
                        value={capturePreview.externalSource || ""}
                        onChange={(e) =>
                          setCapturePreview({
                            ...capturePreview,
                    ...
                    <div className="md:col-span-2">
                      ...
                      {(capturePreview.images || []).length === 0 && (
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
                          No se detectaron imágenes automáticamente.
                        </p>
                      )}
                    </div>
                  </div>
