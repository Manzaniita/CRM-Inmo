  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Ventas</h1>
          <p className="text-slate-500 dark:text-slate-400">Gestión de operaciones de venta en curso y concretadas.</p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          <button
            ...
            className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors"
                      ...
                      <div key={sale.id} onClick={() => handleOpenDetail(sale)}>
                        <Card className="p-4 cursor-pointer hover:border-blue-300 transition-all active:scale-[0.98] group relative">
                          ...
                          <div className="mt-2 pt-3 border-t border-gray-50 flex items-center justify-between">
                            <p className="text-sm font-black text-slate-900 dark:text-slate-100">{formatCurrency(sale.precioAcordado || sale.precioPublicado, sale.moneda)}</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Comprador / Propiedad</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Precio</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Comisión</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Fecha</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredSales.map(sale => {
                  ...
                  const client = clients.find(c => c.id === sale.clientCompradorId);
                  return (
                    <tr key={sale.id} className="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors group cursor-pointer" onClick={() => handleOpenDetail(sale)}>
            ...
            {filteredSales.length === 0 && (
              <div className="py-20 text-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-medium tracking-tight">No se encontraron operaciones de venta.</p>
              </div>
            )}
          </div>
        </Card>
        ...
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 shrink-0">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">#{sale.id.toUpperCase()}</span>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 scrollbar-thin">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {sale.precioOfrecido && (
                    <Card className="p-4 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/20">
                      <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Oferta Recibida</p>
                      <p className="text-base font-black text-slate-700 dark:text-slate-300">{formatCurrency(sale.precioOfrecido, sale.moneda)}</p>
                    </Card>
                  )}
                  {sale.precioAcordado && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card className="p-4 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/20">
                  <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Comisión Estimada</p>
                  <p className="text-base font-black text-slate-700 dark:text-slate-300">{formatCurrency(sale.comisionEstimada, sale.moneda)}</p>
                </Card>
                {sale.fechaReserva && (
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Tareas</p>
                    {relatedTasks.map(t => (
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
                    <p className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Comprador</p>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-bold text-xs">
                        {buyer?.name?.charAt(0) || '?'}
                      </div>
                      <div>
                        <p className="text-sm font-bold text-slate-900 dark:text-slate-100">{buyer?.name || 'Desconocido'}</p>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400">{buyer?.phone || '-'}</p>
                      </div>
                    </div>
                  </div>
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-800">
              ...
              <div className="space-y-4">
                <h3 className="font-black text-slate-900 dark:text-slate-100 text-sm">Fechas Clave</h3>
                <div className="p-4 bg-blue-50/30 rounded-2xl border border-blue-100/50 space-y-4">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-500 dark:text-slate-400 font-medium">Creada:</span>
                    <span className="font-bold text-slate-900 dark:text-slate-100">{formatDate(sale.fechaCreacion)}</span>
                  </div>
                  {sale.fechaReserva && (
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-500 dark:text-slate-400 font-medium">Reserva:</span>
                      <span className="font-bold text-yellow-600">{formatDate(sale.fechaReserva)}</span>
                    </div>
                  )}
                  {sale.fechaEscritura && (
                    <div className="flex justify-between items-center text-xs">
                      <span className="text-slate-500 dark:text-slate-400 font-medium">Escritura:</span>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-3xl relative z-10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom duration-300">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 shrink-0">
          <h2 className="font-black text-xl text-slate-900 dark:text-slate-100">{sale ? 'Editar Operación' : 'Nueva Operación de Venta'}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 scrollbar-thin">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:col-span-2">
              <div>
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Precio Publicado</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Oferta Recibida</label>
                <input
                  type="number"
              ...
              <div className="col-span-2 md:col-span-1">
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Comisión Estimada</label>
                <input
                  type="number"
