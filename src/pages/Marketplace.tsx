...
import { Store, Search, Copy, CheckCircle2, X, Tag } from "lucide-react";
...
export default function Marketplace() {
  ...
  const filtered = properties.filter((p) => {
    const matchesSearch =
      normalizeSearchText(p.title).includes(lowerSearch) ||
  ...
  const setStatus = (prop: Property, status: MarketplaceStatus) => {
    updateProperty({
      ...
      marketplaceLastPublishedAt:
        status === "publicada"
          ? new Date().toISOString()
          : prop.marketplaceLastPublishedAt,
    });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Preparación de publicaciones para Facebook Marketplace.
          </p>
        </div>
      </div>

      <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-xl text-sm text-yellow-800 font-medium">
        Publicación automática pendiente de integración. Por ahora podés copiar
        el texto preparado.
      </div>
              ...
              <div key={prop.id}>
                <Card className="hover:shadow-md transition-all">
                  <div className="flex items-start justify-between">
                    ...
                    <Badge variant={STATUS_VARIANT[mStatus] as any}>
                      {STATUS_LABELS[mStatus]}
                    </Badge>
                  </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex flex-wrap gap-2">
                    {!prop.marketplaceId && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => generateMarketplaceId(prop)}
                      >
                        Generar ID
                      </Button>
                    )}
                    ...
                    {prop.marketplaceId && mStatus !== "publicada" && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setStatus(prop, "publicada")}
                      >
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => copyPublicationText(prop)}
                    >
                      {copiedId === prop.id ? (
                        <CheckCircle2
                          size={14}
