...
import {
  Plus, Search, Sofa, X, Trash2, Edit3, CheckCircle2
} from 'lucide-react';
...
export default function WaitingRoom() {
  ...
  const [editingEntry, setEditingEntry] = useState<WaitingRoomEntry | null>(null);
  ...
  const filtered = waitingRoom.filter(e => {
    const matchesSearch = normalizeSearchText(e.nombre).includes(lowerSearch) ||
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingEntry) {
      updateWaitingRoomEntry({ ...(formData as WaitingRoomEntry), id: editingEntry.id });
    } else {
      addWaitingRoomEntry({ ...(formData as WaitingRoomEntry), id: generateId('w') });
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Sala de Espera</h1>
          <p className="text-slate-500 dark:text-slate-400">Interesados y personas pendientes de atención.</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            {searchTerm || filterStatus ? 'No se encontraron resultados.' : 'La sala de espera está vacía.'}
          </p>
        </div>
      ) : (
          ...
          {filtered.map(entry => (
            ...
            <Card className="hover:shadow-md transition-all">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-slate-100">{entry.nombre}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{entry.telefono} • {entry.email}</p>
                </div>
                <Badge variant={STATUS_VARIANT[entry.estado] as any}>{entry.estado}</Badge>
              </div>
              <div className="mt-3 space-y-1 text-sm">
                <p className="text-slate-600 dark:text-slate-400"><span className="font-semibold text-slate-800 dark:text-slate-200">Interés:</span> {entry.interes || '-'}</p>
                <p className="text-slate-600 dark:text-slate-400"><span className="font-semibold text-slate-800 dark:text-slate-200">Ingreso:</span> {entry.fechaIngreso}</p>
                {entry.notas && <p className="text-slate-500 dark:text-slate-400 text-xs italic">{entry.notas}</p>}
              </div>
              <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex gap-2">
                <button className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" onClick={() => openForm(entry)} title="Editar">
                  <Edit3 size={16} />
                </button>
                <button className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" onClick={() => handleDelete(entry.id)} title="Eliminar">
                  <Trash2 size={16} />
                </button>
              </div>
            </Card>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg relative z-10 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <h2 className="font-bold text-xl text-slate-900 dark:text-slate-100">{editingEntry ? 'Editar' : 'Nuevo'} Ingreso</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Interés</label>
                <input className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20" value={formData.interes} onChange={e => setFormData({...formData, interes: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Estado</label>
                  <select className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20" value={formData.estado} onChange={e => setFormData({...formData, estado: e.target.value as WaitingRoomStatus})}>
                    <option value="pendiente">Pendiente</option>
                    <option value="contactado">Contactado</option>
                    <option value="descartado">Descartado</option>
                    <option value="convertido">Convertido</option>
                ...
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-1">Fecha de ingreso</label>
                  <input type="date" className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20" value={formData.fechaIngreso} onChange={e => setFormData({...formData, fechaIngreso: e.target.value})} />
                </div>
              </div>
              <div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <Button type="submit" variant="primary">{editingEntry ? 'Guardar' : 'Crear'}</Button>
              </div>
            </form>
          </div>
        </div>
