...
export default function Dashboard() {
  ...
  const salesGoalPercent = totalSalesForGoal > 0 ? (closedSales / totalSalesForGoal) * 100 : 0;
    ...
    <div className="space-y-8 pb-10 page-enter">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Este es el resumen de tu actividad inmobiliaria para hoy.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card title="Objetivos de Venta" subtitle="Rendimiento del período" className="lg:col-span-2">
          ...
          <div className="mt-6 space-y-4">
            ...
            />
            <LinearProgress
              percentage={totalProperties > 0 ? (availableProperties / totalProperties) * 100 : 0}
              ...
              {todayEvents.map(event => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 truncate mt-0.5">{event.description || event.notes}</p>
                  ...
                  <div className="p-1 opacity-0 group-hover:opacity-100 transition-opacity self-center">
                    <ChevronRight size={18} className="text-slate-300 dark:text-slate-600" strokeWidth={1.5} />
                  </div>
                </motion.div>
              ))}
              {todayEvents.length === 0 && (
                <div className="text-center py-10">
                  <Calendar size={40} className="mx-auto text-slate-200 dark:text-slate-700 mb-3" strokeWidth={1.5} />
                  <p className="text-slate-400 dark:text-slate-500 text-sm italic">No hay eventos para hoy.</p>
                </div>
              )}
            </div>
          </Card>
            ...
            <div className="space-y-2 pt-2">
              {pendingTasksList.slice(0, 8).map((task, idx) => (
                ...
                >
                  <div className={cn(
                    ...
                    task.status === 'vencida' || isOverdue(task.dueDate) ? "bg-rose-500" :
                    task.status === 'en proceso' ? "bg-accent dark:bg-dark-accent" : "bg-amber-500"
                  )} />
                  <div className="flex-1 min-w-0 cursor-pointer" onClick={() => navigate('/tareas')}>
                    ...
                    <div className="flex items-center mt-1 gap-2">
                      <span className="text-xs text-slate-400 dark:text-slate-500 flex items-center font-medium">
                        ...
                        {isOverdue(task.dueDate) && <span className="ml-1 text-rose-500 font-bold">(Vencida)</span>}
                        {isToday(task.dueDate) && <span className="ml-1 text-amber-500 font-bold">(Hoy)</span>}
                        {task.status === 'reprogramado' && <span className="ml-1 text-violet-500 font-bold">(Reprogramada)</span>}
                      </span>
              ...
              {pendingTasksList.length > 8 && (
                <div className="text-center pt-2">
                  <button onClick={() => navigate('/tareas')} className="text-xs font-bold text-accent dark:text-dark-accent hover:underline">Ver todas las tareas ({pendingTasksList.length})</button>
                </div>
              )}
              {pendingTasksList.length === 0 && (
                <div className="text-center py-8">
                  <CheckSquare size={40} className="mx-auto text-slate-200 dark:text-slate-700 mb-3" strokeWidth={1.5} />
                  <p className="text-slate-400 dark:text-slate-500 text-sm italic">¡Todo al día por aquí!</p>
                </div>
              )}
            </div>
          </Card>
          ...
          <Card title="Clientes destacados" subtitle="Enviados al panel">
            <div className="space-y-3 pt-2">
              {pinnedClients.map(client => (
                <Link key={client.id} to={`/clientes/${client.id}`} className="flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 p-2 rounded-xl transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-slate-100 truncate group-hover:text-accent dark:group-hover:text-dark-accent transition-colors">{client.name}</h4>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 font-black uppercase">{client.type} • {client.origin}</p>
                  </div>
                </Link>
              ))}
              {pinnedClients.length === 0 && (
                <div className="text-center py-6">
                  <Star size={32} className="mx-auto text-slate-200 dark:text-slate-700 mb-2" strokeWidth={1.5} />
                  <p className="text-slate-400 dark:text-slate-500 text-sm italic">No hay clientes destacados.</p>
                </div>
              )}
            </div>
          </Card>
          ...
          <Card title="Seguimiento de Clientes" subtitle="Nuevos contactos">
            <div className="space-y-3 pt-2">
              ...
              {clients.filter(c => c.status === 'nuevo' || c.status === 'contactado').length === 0 && (
                <div className="text-center py-6">
                  <Users size={32} className="mx-auto text-slate-200 dark:text-slate-700 mb-2" strokeWidth={1.5} />
                  <p className="text-slate-400 dark:text-slate-500 text-sm italic">No hay clientes nuevos.</p>
                </div>
              )}
            </div>
          </Card>

          <Card title="Últimos movimientos" className="bg-slate-50/50 dark:bg-slate-900/30">
              ...
              {allMovements.map(mov => (
                <div key={mov.id} className="relative">
                  <div className={cn(
                    ...
                    mov.type === 'sale' || mov.type === 'property' ? "bg-accent dark:bg-dark-accent" :
                    mov.type === 'client' ? "bg-violet-500 dark:bg-violet-400" :
                    mov.type === 'task' ? "bg-amber-500 dark:bg-amber-400" :
                    mov.type === 'event' ? "bg-emerald-500 dark:bg-emerald-400" : "bg-slate-400 dark:bg-slate-500"
                  )} />
                  <p className="text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase">{formatDate(mov.createdAt)}</p>
                  <p className="text-sm text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">
                    <span className="font-bold text-slate-900 dark:text-slate-100">{mov.title}</span>
                    {mov.description && <><br /><span className="text-xs text-slate-500 dark:text-slate-400">{mov.description}</span></>}
                  </p>
                </div>
              ))}
              {allMovements.length === 0 && (
                <div className="text-center py-4">
                  <Activity size={24} className="mx-auto text-slate-200 dark:text-slate-700 mb-2" strokeWidth={1.5} />
                  <p className="text-xs text-slate-400 dark:text-slate-500 italic">No hay movimientos recientes.</p>
                </div>
              )}
            </div>
          </Card>
...
function AlertRow({
  ...
  const colorMap: Record<string, string> = {
    ...
    orange: 'text-amber-600 bg-amber-50 dark:bg-amber-500/10 dark:text-amber-400',
