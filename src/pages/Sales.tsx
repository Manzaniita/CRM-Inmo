  ...
  const filteredSales = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "precio":
          compare =
            (a.precioAcordado || a.precioPublicado) -
            (b.precioAcordado || b.precioPublicado);
          break;
        default:
          compare = 0;
      }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            Gestión de operaciones de venta en curso y concretadas.
          </p>
        </div>
                        ...
                        <Card className="p-4 cursor-pointer hover:border-blue-300 transition-all active:scale-[0.98] group relative">
                          ...
                          <div className="mt-2 pt-3 border-t border-gray-50 flex items-center justify-between">
                            <p className="text-sm font-black text-slate-900 dark:text-slate-100">
                              {formatCurrency(
                                sale.precioAcordado || sale.precioPublicado,
                                sale.moneda,
                              )}
                            </p>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredSales.map((sale) => {
                  ...
                  return (
                    <tr
                      key={sale.id}
                      className="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors group cursor-pointer"
                    ...
                    >
                        ...
                        <div className="flex flex-col">
                          ...
                          {sale.precioAcordado && (
                            <span className="text-[10px] text-green-600 font-medium">
                              Acordado:{" "}
                              {formatCurrency(sale.precioAcordado, sale.moneda)}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <Badge variant={getStatusVariant(sale.estado)}>
                          {sale.estado}
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button className="p-2 text-slate-300 dark:text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
                            <FileText size={18} />
                          </button>
                          <button className="p-2 text-slate-300 dark:text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all">
            ...
            {filteredSales.length === 0 && (
              <div className="py-20 text-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-medium tracking-tight">
                  No se encontraron operaciones de venta.
                </p>
              </div>
            )}
          </div>
...
function SaleDetailModal({
  ...
  const relatedTasks = tasks.filter(
    (t) =>
      t.propertyId === sale.propiedadId &&
      t.clientId === sale.clientCompradorId,
  );
  const relatedEvents = events.filter(
    (e) =>
      e.propertyId === sale.propiedadId &&
      e.clientId === sale.clientCompradorId,
  );
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-4xl relative z-10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 shrink-0">
          <div>
            ...
            <h2 className="font-black text-2xl text-slate-900 dark:text-slate-100">
              {property?.title}
            </h2>
          </div>
          <button
                  ...
                  {sale.precioOfrecido && (
                    <Card className="p-4 border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/20">
                      ...
                      <p className="text-base font-black text-slate-700 dark:text-slate-300">
                        {formatCurrency(sale.precioOfrecido, sale.moneda)}
                      </p>
                    </Card>
                  )}
                  {sale.precioAcordado && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    ...
                    {relatedTasks.map((t) => (
                      <div
                        key={t.id}
                        className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-800 text-xs"
                      >
                        <CheckCircle2
                          size={14}
                          className={
                  ...
                  {sale.fechaEscritura && (
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-bold text-green-600">
                        {formatDate(sale.fechaEscritura)}
                      </span>
                    </div>
                  )}
                </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-3xl relative z-10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom duration-300">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900 shrink-0">
          <h2 className="font-black text-xl text-slate-900 dark:text-slate-100">
            {sale ? "Editar Operación" : "Nueva Operación de Venta"}
          </h2>
          <button
            onClick={onClose}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <SearchableSelect
                ...
                onChange={(val) =>
                  setFormData({ ...formData, clientCompradorId: val })
                }
              ...
              <div className="flex gap-2">
                {["USD", "ARS"].map((m) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, moneda: m as any })
                    }
                    className={cn(
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      precioPublicado: Number(e.target.value),
                    })
                  }
                />
              </div>
              ...
              <div className="col-span-2 md:col-span-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      comisionEstimada: Number(e.target.value),
                    })
                  }
                />
              </div>
