      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg relative z-10 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
          <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <h2 className="font-bold text-xl text-slate-900 dark:text-slate-100">{editingTask ? 'Editar Tarea' : 'Nueva Tarea'}</h2>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <label className="block text-sm font-bold text-slate-700 dark:text-slate-300">Vincular con</label>
        ...
        {statuses.map(status => (
          <div key={status} className="flex-1 min-w-[300px] flex flex-col gap-4">
            <div className="flex items-center justify-between px-2">
              <h3 className="text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{status}</h3>
            ...
            <div className="flex-1 space-y-3 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
              {filteredTasks.filter(t => t.status === status).map(task => (
                ...
                >
                  ...
                  <h4 className="font-bold text-slate-900 dark:text-slate-100 text-sm mb-1">{task.title}</h4>
        ...
        {filteredTasks.map((task) => (
          <div key={task.id} className="p-4 hover:bg-slate-50 dark:bg-slate-800/50 transition-colors flex items-center gap-4 group">
            <button
              ...
              className={cn(
                "transition-colors",
                task.status === 'completada' ? "text-green-500" : "text-slate-300 dark:text-slate-600 hover:text-green-600"
              )}
            >
            ...
            <div className="flex-1 min-w-0 cursor-pointer" onClick={() => handleOpenForm(task)}>
              <div className="flex items-center gap-2 mb-1">
                <h4 className={cn("font-semibold text-slate-900 dark:text-slate-100 truncate", task.status === 'completada' && "line-through text-slate-400 dark:text-slate-500")}>
                  {task.title}
                </h4>
                <Badge variant={getPriorityVariant(task.priority)} size="sm">{task.priority}</Badge>
              </div>
              <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-slate-400 dark:text-slate-500">
                <div className="flex items-center gap-1">
                  <Calendar size={14} />
                  <span className={cn(task.status === 'vencida' ? "text-red-500 font-bold" : "text-slate-500 dark:text-slate-400")}>
                ...
                {task.relatedEntities && task.relatedEntities.map(r => {
                  ...
                  return (
                    <span key={`${r.type}-${r.id}`} className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                      {icon}
                      {label}
                    </span>
                  );
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Organiza tu trabajo diario y seguimiento de clientes.</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Pendientes</p>
          <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">{tasks.filter(t => t.status === 'pendiente').length}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">En Proceso</p>
          <p className="text-2xl font-bold text-blue-600">{tasks.filter(t => t.status === 'en proceso').length}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Urgentes</p>
          <p className="text-2xl font-bold text-red-600">{tasks.filter(t => t.priority === 'urgente' && t.status !== 'completada').length}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Vencidas</p>
          <p className="text-2xl font-bold text-orange-600">{tasks.filter(t => t.status === 'vencida').length}</p>
        </div>
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1">Reprogramadas</p>
