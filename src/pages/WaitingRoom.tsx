    ...
    <div className="space-y-6 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500">Interesados y personas pendientes de atención.</p>
        </div>
          ...
          {filtered.map(entry => (
            ...
            <Card className="hover:shadow-md transition-all">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-bold text-gray-900">{entry.nombre}</h3>
                  <p className="text-xs text-gray-500 mt-0.5">{entry.telefono} • {entry.email}</p>
                </div>
                <Badge variant={STATUS_VARIANT[entry.estado] as any}>{entry.estado}</Badge>
              </div>
              <div className="mt-3 space-y-1 text-sm">
                <p className="text-gray-600"><span className="font-semibold text-gray-800">Interés:</span> {entry.interes || '-'}</p>
                <p className="text-gray-600"><span className="font-semibold text-gray-800">Ingreso:</span> {entry.fechaIngreso}</p>
                {entry.notas && <p className="text-gray-500 text-xs italic">{entry.notas}</p>}
              </div>
              <div className="mt-4 pt-3 border-t border-gray-100 flex gap-2">
                <button className="p-1.5 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" onClick={() => openForm(entry)} title="Editar">
                  <Edit3 size={16} />
                </button>
                <button className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" onClick={() => handleDelete(entry.id)} title="Eliminar">
                  <Trash2 size={16} />
                </button>
              </div>
            </Card>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg relative z-10 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h2 className="font-bold text-xl text-gray-900">{editingEntry ? 'Editar' : 'Nuevo'} Ingreso</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto max-h-[70vh]">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">Interés</label>
                <input className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20" value={formData.interes} onChange={e => setFormData({...formData, interes: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Estado</label>
                  <select className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20" value={formData.estado} onChange={e => setFormData({...formData, estado: e.target.value as WaitingRoomStatus})}>
                    <option value="pendiente">Pendiente</option>
                    <option value="contactado">Contactado</option>
                    <option value="descartado">Descartado</option>
                    <option value="convertido">Convertido</option>
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Fecha de ingreso</label>
                  <input type="date" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500/20" value={formData.fechaIngreso} onChange={e => setFormData({...formData, fechaIngreso: e.target.value})} />
                </div>
              </div>
              <div>
